...
Programmers of CUDA Quantum may access the IQM Server from either C++ or Python. Install `iqm-cortex-cli`,
follow the `quick start guide <https://iqm-finland.github.io/cortex-cli/readme.html#using-cortex-cli>`__ to login and initialize the tokens file.
...
pass the flag ``--target iqm`` to the ``nvq++`` compiler. There is one mandatory string argument `--iqm-qpu-architecture`.
The provided architecture must match the device architecture the compiled programs is being executed against.
The hardware architecture for a specific IQM Server can be checked  via `https://<IQM server>/cocos/quantum-architecture`.
...
.. code:: bash
    ...
    # executing the same program against an IQM Server with a different underlying QPU architecture will result in an error
